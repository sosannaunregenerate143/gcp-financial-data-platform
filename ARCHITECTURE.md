...
flowchart TB
    ...
    subgraph EventBus["Event Bus"]
        VALID_T["Pub/Sub: validated-events"]
        DLQ_T["Pub/Sub: dead-letter-queue"]
    end
    ...
    subgraph DataWarehouse["Data Warehouse"]
        BQ_INT["BigQuery: intermediate.*\nint_daily_revenue\nint_customer_usage\nint_cost_by_center"]
        BQ_MARTS_F["BigQuery: marts_finance.*\nfct_daily_revenue_summary\nfct_monthly_cost_attribution\nfct_revenue_by_product_region"]
        BQ_MARTS_A["BigQuery: marts_analytics.*\nfct_customer_usage_report\nfct_unit_economics"]
        BQ_AUDIT["BigQuery: audit.*\naccess_log\npermission_changes\npipeline_audit_log\nanomaly_alerts"]
    end
    ...
    subgraph DurableStorage["Durable Storage"]
        ...
        BQ_SNAP["BigQuery Snapshots\n(daily, 30d retention)"]
    end

    BILL & USAGE & COST -->|"POST /api/v1/events\n(JSON)"| API
    VAL -->|"Valid"| VALID_T
    VAL -->|"Invalid"| DLQ_T
    API -->|"Dual write"| BIGTABLE
    ...
    BQ_MARTS_F & BQ_MARTS_A -->|"Access controlled"| GOV

    GCS_RAW -->|"Storage Transfer\n03:00 UTC daily"| GCS_BAK
    BQ_MARTS_F -->|"Data Transfer\nService"| BQ_SNAP

    style WritePath fill:#e8f5e9,stroke:#388e3c
    style BatchPath fill:#e3f2fd,stroke:#1976d2
    style ReadPath fill:#fff3e0,stroke:#f57c00
    style HotStore fill:#fce4ec,stroke:#c62828
    style DurableStorage fill:#f3e5f5,stroke:#7b1fa2
```
